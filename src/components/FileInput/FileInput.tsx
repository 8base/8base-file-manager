...
import { MODAL_STYLE } from '../Modal/Modal.style';
          ...
          { props.children({ openModal: openModal, value, originalFile, error }) }
          {
            state.isModalOpen && (
              ...
              <div className={MODAL_STYLE.overlay}>
                <div className={MODAL_STYLE.content}>
                   <FileChooser
                    apiKey={apiKey}
                    workspace={workspace}
                    environment={environment}
                    uploadHost={uploadHost}
                    onUploadDone={(item, _originalFile) => {
                      let result: FileInputValue = item;
                      if (typeof onUploadDone === 'function') {
                        result = props.onUploadDone(item, originalFile);
                       }
                      setState((prevState) => ({ ...prevState, value: item, originalFile: originalFile}));
                      closeModal();
                      return result;
                    }}
                    onChange={(value, originalFile) => {
